...
public:
    ...
    static void  init( const int argc, const char** argv, const char * init_args = 0);
...
private:
    ...
    void  createArgsMaps( const int argc, const char** argv, const char *init_args);
...
CmdArgReader::getArgHelper( const std::string& name) 
{
    ...
    fprintf(stderr, "Input argument not found:%s\n", name.c_str());
    RUNTIME_EXCEPTION("Input argument not found:"+ name);
    return NULL;
}
...
inline const std::string &GET_ARG_STRING(const char *name) {return *CmdArgReader::getArg<std::string>(name);}
inline int GET_ARG_INT(const char *name) {return  *CmdArgReader::getArg<int>(name);}
inline float GET_ARG_FLOAT(const char *name) {return  *CmdArgReader::getArg<float>(name);}

inline std::string GET_ARG_STRING(const char *name, const char *init)
	{ return CmdArgReader::existArg(name)? *CmdArgReader::getArg<std::string>(name) : init;}
inline int GET_ARG_INT(const char *name, int init)
	{ return CmdArgReader::existArg(name)? *CmdArgReader::getArg<int>(name) : init;}
inline float GET_ARG_FLOAT(const char *name, float init)
	{ return CmdArgReader::existArg(name)? *CmdArgReader::getArg<float>(name) : init;}
